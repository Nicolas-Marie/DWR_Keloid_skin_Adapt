...
Calcul et étude de la solution duale
...
#Définition des fonctions d'essai z et test v
z = TrialFunction(V)
v = TestFunction(V)

#forme bilinéaire du problème dual
a = inner(grad(z),grad(v))*dx

#Re-définition de z pour la résolution du problème
z = Function(V)
...
"""
Résolution du probleme dual
"""

problem_D = LinearVariationalProblem(a, J_1(v), z, bc) #définition du problème
solver_D = AdaptiveLinearVariationalSolver(problem_D, J_1(z)) #définition du solveur, en spécifiant la quantité d'intérêt J
solver_D.parameters["error_control"]["dual_variational_solver"]["linear_solver"] = "cg" # définition des paramètres
solver_D.solve(tol) #appel de la fonction pour la résolution, avec une tolerance donnée

solver_D.summary() #Affichage des résultats

# Récupération des graphes de la solution :

#Sur le maillage initial
vtkfile3 = File("solution_dual_initial_mesh.pvd")
vtkfile3 << z.root_node()

#Sur le maillage final
vtkfile4 = File("solution_dual_final_mesh.pvd")
vtkfile4 << z.leaf_node()
